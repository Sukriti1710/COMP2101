...
for arrayint in $interface;do
  echo "$arrayint"
   if [[ $arrayint = lo* ]] ;
     then
	  continue
   fi
    ipv4_address=$(ip a s $arrayint | awk -F '[/ ]+' '/inet /{print $3}')
    ipv4_hostname=$(getent hosts $ipv4_address | awk '{print $2}')
    network_address=$(ip route list dev $arrayint scope link | cut -d ' ' -f 1)
    network_number=$(cut -d / -f 1 <<<"$network_address")
    network_name=$(getent networks $network_number | awk '{print $1}')
	if [ -z "$network_name" ]
      then
	  network_name="No network_name found."
    else
      echo 
    fi
    echo Interface $arrayint:
    echo =================
